...
- 메뉴의 `プライバシーポリシー`에서 HTTPS URL을 시스템 기본 외부
  브라우저로 표시하고, 열기 실패 시 일본어 오류 안내를 제공합니다.
- 현재 앱 코드는 폐기 예정인 이전 ChatGPT Sites URL을 참조합니다. 새 공개
  호스팅과 URL을 사용자가 결정해 앱 링크를 교체하고 비로그인 접근을 검증하기
  전에는 출시할 수 없습니다.
