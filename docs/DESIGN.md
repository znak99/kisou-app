...
- 외부 웹 삭제용 `サポートID`와 `アカウント削除用コード`를 사용자가
  명시적으로 발급하며, 코드 원문은 해당 단말의 보안 저장소에만 보관
...
| GET | /users/me/account-deletion-credentials | 지원 ID·코드 버전·발급 상태 조회(코드 원문 제외) |
| POST | /users/me/account-deletion-credentials/rotate | 삭제용 코드 최초 발급 또는 교체, 새 원문 1회 반환 |
...
- Apple/Google 계정 연동 상태 표시 코드는 유지하지만 신규 연동 행동은
  운영판에서 비활성화
- 연결 계정 로그아웃
- 외부 삭제용 지원 ID·삭제 코드 관리
- 계정 삭제

`削除用情報`는 200% 글자 크기에서도 가로 스크롤이 필요 없는 독립 스크롤
화면으로 제공합니다.

- 진입할 때 GET descriptor를 조회합니다. 서버에서 아직 발급하지 않은
  계정은 사용자가 확인창에서 `削除用コードを発行`을 선택해야만 rotate
  요청을 보냅니다.
- 서버의 `code_version`과 단말 보안 저장소의 버전이 다르거나 단말 원문이
  없으면 기존 원문은 복원할 수 없다고 표시하고, 자동 발급 없이 명시적인
  `新しいコードに置き換える`을 제공합니다.
- Keychain·Keystore 읽기에서 원인이 확정되지 않은 플랫폼 오류가 발생하면
  원문을 자동 삭제하지 않습니다. 재읽기를 먼저 제공하고, 계속 실패할 때만
  경고 확인을 거쳐 단말 코드만 명시적으로 폐기한 뒤 교체 화면으로 전환합니다.
- 지원 ID는 공개 식별자로 항상 표시하고 일반 복사를 허용합니다. 삭제 코드
  원문은 기본 마스킹하고 표시·복사는 각각 명시적 동작으로만 수행합니다.
  앱이 백그라운드로 이동하면 즉시 다시 마스킹하고, 삭제 코드가 그대로인
  클립보드는 최대 60초 뒤 또는 화면 수명 종료 시 지웁니다. 비동기 보안
  저장소 읽기·클립보드 쓰기 도중 화면이 비활성화되더라도 작업 세대와
  resumed 상태를 다시 검사해 뒤늦게 원문이 표시·잔류하지 않게 합니다.
  클립보드 쓰기는 직렬화해 이전 정리가 새 복사를 덮지 못하게 합니다.
- 삭제 코드 원문은 Riverpod 상태, 사용자 모델, 로그, `SharedPreferences`에
  넣지 않습니다. `flutter_secure_storage`에 지원 ID·버전을 함께 저장하고,
  iOS는 iCloud 동기화가 없는 `unlocked_this_device`, Android는 Keystore
  기반 암호화를 사용합니다. 클립보드 정리 비교에는 원문 대신 SHA-256
  지문만 메모리에 유지합니다.
- 사용자가 `安全な場所に保存しました`를 눌러야 현재 코드 버전의 백업이
  확인됩니다. 단순 복사는 백업 완료로 간주하지 않으며 코드를 교체하면 이전
  확인과 코드가 즉시 무효화됩니다.
- 같은 사용자 ID를 유지하는 익명 계정 연동에는 삭제용 정보를 보존하지만,
  연결 계정에서 완전히 로그아웃할 때는 다른 계정으로 전환하기 전에 단말의
  삭제 코드와 백업 확인을 제거합니다. 세션 만료 뒤 로그인을 선택한 경우에는
  새 로그인이 성공한 다음 이전 계정의 단말 삭제용 정보를 제거하고 사용자별
  provider 캐시를 무효화합니다.
- 앱 재설치·기기 분실·고장·기기 변경 시 단말 원문을 잃을 수 있으므로
  패스워드 관리 앱 등 별도의 안전한 장소에 두 값을 보관하도록 안내합니다.
  지원 ID와 코드를 함께 가진 사람은 영구 삭제할 수 있지만, 코드는 로그인이나
  데이터 조회에는 사용할 수 없음을 같은 화면에서 경고합니다.
...
- 앱을 사용할 수 없는 경우 공개 삭제 페이지에서 지원 ID와 삭제 코드를
  검증한 뒤, 별도의 최종 확인을 거쳐 계정을 삭제할 수 있습니다. 삭제 코드는
  충분한 엔트로피의 서버 난수로 발급하고 서버에는 원문 대신 단방향 해시와
  버전만 저장합니다.
- 삭제 코드 최초 발급과 교체는 인증된 앱에서만 수행하며, 교체 시 이전 코드와
  이전 백업 확인을 즉시 무효화합니다. GET descriptor와 앱 상태에는 원문을
  반환하거나 보관하지 않습니다.
...
- DELETE 401, status 404·429·5xx·파싱 오류·네트워크 오류는 삭제 성공
  증거가 아니므로 marker, token이 아닌 계정 로컬 데이터, 여행 일정과
  삭제용 정보를 보존합니다. 401 interceptor와 동시 세션 만료도 삭제
  marker를 account switch로 덮지 않습니다. 익명 계정은 기존
  `device_secret`으로 같은 계정만 엄격 복원하고 새 guest 생성 fallback은
  사용하지 않습니다. 복원할 수 없으면 저장한 지원 ID·삭제 코드로 공개
  삭제 페이지를 이용하도록 복구 화면에서 안내합니다.
- 원계정 세션을 복원할 수 없고 status의 최신 결과가 네트워크 오류나
  429·5xx가 아닌 확정 404일 때만 `이 기기의 데이터만 삭제` 보조 동작을
  표시합니다. 확인 창은 서버 삭제 미확인·서버 데이터 잔존 가능성,
  지원 ID·삭제 코드가 있으면 Web 삭제를 먼저 수행할 것, 로컬 삭제는
  복구할 수 없음을 명시합니다. 사용자가 확인해도 서버 삭제 성공으로
  표시하거나 DELETE를 다시 보내지 않습니다. 먼저 crash-safe
  `unconfirmedAccountDiscard` marker로 전환한 뒤 여행·알림·삭제 코드·테마와
...
- 매일 푸시를 사용한 단말은 계정 삭제 전 더 높은 unregister revision을
  ...
  연결된 외부 live·backup 데이터 삭제에는 최대 180일이 걸릴 수 있으며,
  공개 웹 삭제는 분실·고장 단말의 로컬 token/FID를 직접 삭제할 수 없습니다.
