...
기존 결정에 우선하는 최신 UI 조정 사항.

1. **앱 아이콘 변경 알럿 제거 → 자동 스왑 폐기**: iOS 26에서는 앱 아이콘 변경 시 시스템 알럿("아이콘을 변경했습니다")이 **프로세스 외부에서 강제 표시**되어 앱에서 억제할 수 없다(비공개 API·present 스위즐 모두 실패 확인). '팝업 미표시'를 우선하여 **인앱 테마 기반 아이콘 자동 변경 기능을 제거**하고, **단일 정적 아이콘(라이트 디자인)**만 사용한다. 대체 아이콘(AppIconDark)·스왑 로직·네이티브 설정은 모두 삭제.
2. **홈 좌상단 로고 확대**: 홈 화면 좌상단 로고(이미지+텍스트)를 기존 대비 **15% 확대**.
3. **전 탭 로고 배치**: 홈뿐 아니라 **모든 탭(홈/분석/메뉴) 좌상단에 동일 로고**를 배치.
4. **오늘의 날씨 카드 축소**: 오늘의 날씨 카드와 내부 컴포넌트(온도·체감·지표) 크기를 **약 5% 축소**. 레이아웃 깨짐(오버플로) 주의.
5. **체감 문구 그래디언트**: 예상 체감 카드 배경 그래디언트를 **위→아래 방향**으로 하고, 양끝 색 차이를 **더 줄여** 은은하게.
6. **복장 추천 더보기 토글**: 복장 추천 영역 **우측 하단에 '더보기' 토글 버튼**을 두어, 2·3순위(少し暖かめ / 少し軽め)를 접었다 폈다 할 수 있게 한다(기본 접힘).
7. **탭 명칭/아이콘 변경**: '프로필' 탭을 **'메뉴(メニュー)'**로 변경하고, 아이콘을 **가로 점 3개(more_horiz)**로 교체. 화면 제목도 동일하게 변경.

# 리뷰 v2.3 (공유 상단 툴바)

1. **좌상단 로고 10% 확대**: 전 탭 좌상단 로고(이미지+텍스트) 크기를 기존 대비 **10% 확대**(size 32 → 35).
2. **탭바 공유 상단 툴바 도입**: v2.2에서 각 화면이 **개별적으로** 배치하던 로고를, 루트 셸(`root_shell`)의 **공유 상단 툴바(`KisouTopBar`)**로 승격한다. 툴바 = `SafeArea(top)` 안에 좌측 로고 + `Spacer` + **우측 탭별 액션 슬롯**. 탭 전환과 무관하게 로고가 고정되고, 각 화면(홈/분석/메뉴)은 상단 로고를 더 이상 직접 그리지 않는다.
   - **우측 액션 버튼**: 툴바 우측에 탭별 액션 위젯을 배치할 수 있다. **홈 탭**은 **피드백 버튼**(`FeedbackActionButton`, 프로바이더 자체 구독 → 제출 상태 반영)을 액션으로 둔다. 분석·메뉴 탭은 현재 액션 없음(로고만).
   - 로고가 항상 툴바에 있으므로 각 화면의 로딩·에러 상태에서도 로고가 일관되게 노출된다.
