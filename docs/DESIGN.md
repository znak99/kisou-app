...
- 현재 Firebase Analytics와 광고 SDK를 설치하지 않았고 IDFA를 수집하거나
  앱·웹 간 추적에 사용하지 않으므로 ATT 팝업을 표시하지 않습니다.
- 향후 분석 또는 광고 SDK를 도입할 때에는 수집 항목과 추적 여부를 다시
  검토한 뒤 ATT와 스토어 개인정보 표시를 갱신합니다.
...
- **공통 셸:** `RootShell`이 홈·예보·메뉴 세 탭, 공통 `KisouTopBar`와
  하단 내비게이션을 관리합니다. 실제 광고 SDK·노출·통신이 없는 미완성 광고
  자리표시자와 날짜 지정 추가 횟수 안내는 개발 환경에서만 표시하고
  운영판에서는 렌더링하지 않습니다.
...
- 날짜 지정 스토어 캡처는 `kDebugMode`와 명시적
  ...
  선택하고 고정 결과와 메모리 내 3회 사용량을 제공합니다. 이 경로는 API와
  저장된 사용량을 건드리지 않으며 production debug에서도 재현할 수 있지만
